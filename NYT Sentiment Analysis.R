...
# Model Building    
  NYT.glm <- glm(critics_pick ~ 
                  sentiment_score, 
                family = "binomial", data = NYT_review_sentiment)
  
  summary(NYT.glm)
  1-pchisq(76.703-72.689 , df = 64 -53)

# Log Odds
  exp(NYT.glm$coefficients)
  
# Means
  mean(NYT_review_sentiment$sentiment_score)
  
  newdata <- data.frame(sentiment_score = seq(0,1,by=0.01))
  pred.vals <- predict(NYT.glm, newdata, type = "response")
  
# Plot - Critics Pick by Sentiment Score   
  plot(x = NYT_review_sentiment$sentiment_score,
       y = NYT_review_sentiment$critics_pick,
       type = "p",
       main = "Critick Pick by Sentiment Score",
       xlab = "Sentiment Score",
       ylab = "Critics Pick")
  abline(h=c(0,1),col="gray75")
  
  inverse.logit.sentiment_score <- exp(pred.vals)/(1+exp(pred.vals))
  lines(seq(0,1, by=0.01), inverse.logit.sentiment_score, col = "blue")
...
# Density Plot of Sentiment scores  
  p <- ggplot(NYT_review_sentiment, aes(x = sentiment_score)) + 
      ...
      facet_wrap(~genre) +
      ggtitle("Density Plot of Sentiment Scores") +
...
# Histogram Plot of Sentiment Scores
  p <- ggplot(NYT_review_sentiment, aes(x = sentiment_score)) + 
    ...
    facet_wrap(~genre) +
    ggtitle("Histogram of Sentiment Scores") +
    scale_y_continuous("Density") +
    scale_x_continuous("Sentiment Score") +
    labs(colour = "Genre") +
    theme_bw() +
